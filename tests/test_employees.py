...
async def test_list_employees_pagination(async_client: AsyncClient):
    ...
    for i in range(5):
        await async_client.post(
            "/api/v1/employees", json={"name": f"P{i}", "rfid_uid": f"PAGE-{i:03d}"}
        )
...
async def test_get_employee_by_id(async_client: AsyncClient):
    ...
    create = await async_client.post(
        "/api/v1/employees", json={"name": "Solo", "rfid_uid": "SOLO-001"}
    )
    eid = create.json()["id"]
...
async def test_update_employee(async_client: AsyncClient):
    ...
    create = await async_client.post(
        "/api/v1/employees", json={"name": "Old Name", "rfid_uid": "UPD-001"}
    )
    eid = create.json()["id"]
    resp = await async_client.put(
        f"/api/v1/employees/{eid}", json={"name": "New Name", "department": "Sales"}
    )
    assert resp.status_code == 200
    data = resp.json()
...
async def test_delete_employee_soft(async_client: AsyncClient):
    ...
    create = await async_client.post(
        "/api/v1/employees", json={"name": "Del Me", "rfid_uid": "DEL-001"}
    )
    eid = create.json()["id"]
